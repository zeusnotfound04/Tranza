  ...
  try {
    ...
    await respond({
      text: `✅ *Money sent successfully!*\n\n` +
            ...
            `💵 *You sent:* ₹${transfer.amount} to ${recipient}\n` +
            `📊 *Service fee:* ₹${(parseFloat(transfer.total_amount) - parseFloat(transfer.amount)).toFixed(2)}\n` +
            `💳 *Total charged:* ₹${transfer.total_amount}\n\n` +
            `📋 *For your records:*\n` +
            `Reference: \`${transfer.reference_id}\`\n` +
            `Transaction ID: \`${transfer.transfer_id}\`\n` +
            `Completed at: ${currentTime}\n\n` +
            `✔️ *All done!* Your recipient should receive the money shortly. Thanks for using Tranza!`,
